...
/*   Updated: 2024/07/22 06:54:43 by mkimdil          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *handle_single_quotes(char *current, char *cmd, int *j) {
    (*j)++;
    while (current[*j] && current[*j] != '\'') {
        cmd = ft_strjoin(cmd, ft_substr(current, *j, 1));
        (*j)++;
...
char *handle_double_quotes(char *current, char *cmd, t_list *envp, int *j) {
    int k;
    char *var_name;
    char *value;
...
char *handle_variable_expansion(char *current, char *cmd, t_list *envp, int *j) {
    int k;
    char *var_name;
    char *value;
...
char *expand_cmd(t_cmd *lst, t_list *envp, int i) {
    ...
    while (current[j]) {
        if (current[j] == '$' && current[j + 1] == '?')
            ...
            cmd = handle_single_quotes(current, cmd, &j);
        else if (current[j] == '"')
            cmd = handle_double_quotes(current, cmd, envp, &j);
        else if (current[j] == '$' && special_case(current[j + 1]))
            cmd = handle_variable_expansion(current, cmd, envp, &j);
        else if (current[j] == '$' && current[j + 1] == '$')
            j += 2;
        else if (current[j] == '$' && current[j + 1] == '"')
            j++;
...
void update_argv_with_expanded(t_cmd *lst, char **splited, int i, int argv_size, int j) {
    int k;
...
void handle_ambiguous_cases(t_cmd *lst, char *expanded, int tr, int i) {
    int argv_size = 0;
    char **splited;
    int j;
    ...
    if (ft_strsearch(expanded, ' ') && tr == 0) {
        ...
        while (splited[j])
            j++;
        update_argv_with_expanded(lst, splited, i, argv_size, j);
    } else {
        lst->argv[i] = ft_strdup(expanded);
        if (ft_strlen(expanded) == 0 && lst->ambiguous == 0) {
            if (tr != 1 && tr != 2)
    ...
    while (lst) {
        ...
        while (lst->argv[i]) {
            if (ft_strchr(lst->argv[i], '$') && lst->argv[i + 1]
                ...
                tmp = lst->argv[i + 1];
            if (ft_strchr(lst->argv[i], '$')) {
                tr = (ft_strsearch(lst->argv[i], '"')) ? 1 : (ft_strsearch(lst->argv[i], '\'')) ? 2 : 0;
                if (tr == 1 || tr == 0) {
                    expanded = expand_cmd(lst, envp, i);
                    handle_ambiguous_cases(lst, expanded, tr, i);
                }
            }
            if (tmp && !lst->argv[i])
                lst->argv[i] = ft_strdup(tmp);
